...
import { Hash, Wallet, Activity, Package } from "lucide-react";
import { useApp } from "../context/AppContext";

export default function PortfolioSummary() {
  const { totalPortfolioValue, totalIdleValue, totalActiveValue, tokens } =
    useApp();
  ...
  return (
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
      {stats.map(({ label, value, icon: Icon, color, textColor, isCount }) => (
        ...
        >
          <div
            className={`absolute -right-4 -top-4 h-16 w-16 rounded-full bg-gradient-to-br ${color} opacity-10 transition-opacity group-hover:opacity-20`}
          />
          <div className="flex items-start justify-between">
            ...
            <div
              className={`rounded-lg bg-gradient-to-br ${color} p-2 opacity-80`}
            >
