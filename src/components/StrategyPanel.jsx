...
import { Sliders, Zap, TrendingUp, HeartPulse } from "lucide-react";
...
const trackConfig = {
  yield: {
    label: "Yield",
    icon: Zap,
    color: "bg-sentinel-success",
    accent: "text-sentinel-success",
  },
  trade: {
    label: "Trade",
    icon: TrendingUp,
  ...
  return (
    <div className="rounded-xl border border-sentinel-border bg-sentinel-card">
      ...
      <div className="p-5 space-y-5">
        ...
        <div>
          <p className="mb-2 text-xs text-sentinel-muted uppercase tracking-wider">
            Risk Profile
          </p>
              ...
              <div key={key}>
                <div className="flex items-center justify-between mb-1.5">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-medium text-sentinel-text">
                      {cfg.label}
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`text-xs font-bold font-mono ${cfg.accent}`}
                    >
                      {value}%
                    </span>
                  </div>
        ...
        <div className="h-3 flex rounded-full overflow-hidden">
          <div
            className="bg-sentinel-success transition-all duration-300"
            style={{ width: `${allocation.yield}%` }}
          />
          <div
            className="bg-sentinel-accent transition-all duration-300"
            style={{ width: `${allocation.trade}%` }}
          />
          <div
            className="bg-amber-400 transition-all duration-300"
            style={{ width: `${allocation.hedge}%` }}
          />
        ...
        <div className="flex items-center justify-between text-[10px] uppercase tracking-wider text-sentinel-muted">
          <span>
            Total: {allocation.yield + allocation.trade + allocation.hedge}%
          </span>
          <span>
            Idle: $
            {totalIdleValue.toLocaleString(undefined, {
              maximumFractionDigits: 0,
            })}
          </span>
        </div>
