...
function StepIcon({ status }) {
  if (status === "complete")
    return <CheckCircle className="h-4 w-4 text-sentinel-success" />;
  if (status === "executing")
    return <Loader2 className="h-4 w-4 text-sentinel-accent animate-spin" />;
  if (status === "failed")
    return <XCircle className="h-4 w-4 text-sentinel-danger" />;
  if (status === "skipped")
...
export default function SimulationPanel() {
  const { simulationSteps, isSimulating, isExecuting, simulate, execute } =
    useApp();
      ...
      <div className="p-5">
        {isSimulating && (
          <div className="flex flex-col items-center justify-center py-10">
            ...
            <p className="text-xs text-sentinel-muted mt-1">
              Analyzing routes and protocols
            </p>
          </div>
        )}
        ...
        {!isSimulating && simulationSteps.length === 0 && (
          <div className="flex flex-col items-center justify-center py-10 text-center">
            ...
            <p className="text-xs text-sentinel-muted mt-1">
              Adjust your allocation and click Simulate to preview the strategy
            </p>
          </div>
        )}
                ...
                className={`relative rounded-lg border p-3.5 transition-all ${
                  step.status === "complete"
                    ...
                    : step.status === "executing"
                      ...
                      : step.status === "failed"
                        ? "border-sentinel-danger/30 bg-sentinel-danger/5"
                        : step.status === "skipped"
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span
                        className={`rounded px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wider border ${trackColors[step.track] || "border-sentinel-border text-sentinel-muted"}`}
                      >
                        {step.track}
                      </span>
                      ...
                      {step.status === "failed" && (
                        <span className="text-[10px] text-sentinel-danger font-semibold">
                          FAILED
                        </span>
                      )}
                      {step.status === "skipped" && (
                    ...
                    <p className="text-sm text-sentinel-textBright">
                      {step.action}
                    </p>
