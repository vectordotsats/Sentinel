...
import { ScrollText, Trash2, ExternalLink, Zap, Play, Monitor } from 'lucide-react'
    ...
    <div className="rounded-xl border border-sentinel-border bg-sentinel-card">
      <div className="flex items-center justify-between border-b border-sentinel-border px-5 py-3.5">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-sentinel-textBright">Execution Log</h3>
        ...
        {executionLog.length === 0 ? (
          ...
          <div className="divide-y divide-sentinel-border">
            {executionLog.map(entry => {
              ...
              return (
                <div key={entry.id} className="flex items-start gap-3 px-5 py-2.5 hover:bg-white/[0.02]">
                  <Icon className={`mt-0.5 h-3.5 w-3.5 flex-shrink-0 ${config.color}`} />
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-sentinel-text truncate">{entry.message}</p>
                    <div className="flex items-center gap-2 mt-0.5">
                      <span className="text-[10px] text-sentinel-muted">{time}</span>
                      {entry.txHash && (
