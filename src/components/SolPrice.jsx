...
export default function SolPrice() {
  ...
  const fetchPrice = async () => {
    ...
    setError(false)
    try {
      const res = await fetch(PROXY_API('https://api.coingecko.com/api/v3/simple/price?ids=solana&vs_currencies=usd&include_24hr_change=true'))
    ...
    <div className="flex items-center gap-3 rounded-xl border border-sentinel-border bg-sentinel-card px-4 py-3">
      ...
      <div>
        <div className="flex items-center gap-2">
          ...
          {change24h !== null && (
            <span className={`flex items-center gap-0.5 text-xs font-medium ${isPositive ? 'text-sentinel-success' : 'text-sentinel-danger'}`}>
